...
namespace XstReader
{
    ...
    public class Folder
    {
        ...
        public List<Folder> Folders => GetFolders();
        ...
        #region Ctor
        internal Folder(XstFile xstFile, NID nid, Folder parentFolder = null)
        {
            XstFile = xstFile;
            Nid = nid;
            ParentFolder = parentFolder;
            Ltp.ReadProperties<Folder>(nid, pgFolder, this);
        }
        #endregion Ctor
        ...
        public List<Folder> GetFolders()
        {
            if (_Folders == null)
                _Folders = Ltp.ReadTableRowIds(NID.TypedNID(EnidType.HIERARCHY_TABLE, Nid))
                              ...
                              .Select(id => new Folder(XstFile, id, this))
                              .OrderBy(sf => sf.Name)
                              .ToList();
        ...
        public List<Message> GetMessages()
        {
            if (_Messages == null)
            {
                if (ContentCount > 0)
                    ...
                    _Messages = Ltp.ReadTable<Message>(NID.TypedNID(EnidType.CONTENTS_TABLE, Nid),
